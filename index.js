...
Object.prototype.onSubmit = function () {
    ...
    if (!formData.valid) {
        ...
        if (!beforeAlert) {
            this.showAlert({ type: 'danger', message: 'Form is invalid, Please fill all required field'});
        }
        return false;
    }
    else {
...
Object.prototype.showAlert = function ({type, message}) {
    let elementID = GenerateID({ length: 12});
...
Object.prototype.isFilled = function () {
    try {
        if (this.hasAttribute('required') && this.value === '') {
            this.addClass({ className: 'error-empty'});
            return false;
        }
        else {
            this.removeClass({ className: 'error-empty'});
            return true;
        }
    } catch (error) {
        return false;
    ...
    try {
        ...
        if (regexPattern !== '' && this.value != '') {
            if (!RegExp(regexPattern).test(this.value)) {
                this.addClass({ className: 'error-invalid'});
                return false;
            }
            else {
                this.removeClass({ className: 'error-invalid'});
                return true;
            }
        }
        else {
